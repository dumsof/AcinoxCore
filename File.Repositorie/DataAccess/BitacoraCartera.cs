using System;
using System.Collections.Generic;

namespace File.Repositorie.DataAccess
{
    public partial class BitacoraCartera
    {
        public string IdModulo { get; set; }
        public string IdTerc { get; set; }
        public string IdSuc { get; set; }
        public string IdCuenta { get; set; }
        public string IdEmp { get; set; }
        public string IdCo { get; set; }
        public string LapsoDoc { get; set; }
        public string IdTipdoc { get; set; }
        public string IdNrodoc { get; set; }
        public string IdCuota { get; set; }
        public string IdFecha { get; set; }
        public string IdConsec { get; set; }
        public string Detalle1 { get; set; }
        public string Detalle2 { get; set; }
        public string Detalle3 { get; set; }
        public string Detalle4 { get; set; }
        public string Detalle5 { get; set; }
        public string Detalle6 { get; set; }
        public string Detalle7 { get; set; }
        public string Detalle8 { get; set; }
        public string Detalle9 { get; set; }
        public string Detalle10 { get; set; }
        public string Hora { get; set; }
        public string Clave { get; set; }
        public string UsuarioIng { get; set; }
        public string FecsisIng { get; set; }
        public string HorsisIng { get; set; }
        public string UsuarioMod { get; set; }
        public string FecsisMod { get; set; }
        public string HorsisMod { get; set; }
        public string FecRadic { get; set; }
        public string DescClave { get; set; }
        public string DescTerc { get; set; }
        public string DirTerc { get; set; }
        public string CiuTerc { get; set; }
        public string IdFechaDcto { get; set; }
    }
}
