using System;
using System.Collections.Generic;

namespace File.Repositorie.DataAccess
{
    public partial class DocumentoVentas
    {
        public string LapsoDoc { get; set; }
        public string IdEmp { get; set; }
        public string IdCo { get; set; }
        public string IdTipdoc { get; set; }
        public string DocumentoFc { get; set; }
        public double? DctoId { get; set; }
        public string FechaDcto { get; set; }
        public string FechaVcto { get; set; }
        public string IdTerc { get; set; }
        public string IdSuc { get; set; }
        public decimal? VlrBruto { get; set; }
        public decimal? VlrDsctoLinea1 { get; set; }
        public decimal? VlrDsctoLinea2 { get; set; }
        public decimal? VlrDsctoGloba1 { get; set; }
        public decimal? VlrDsctoGloba2 { get; set; }
        public decimal? VlrIva { get; set; }
        public decimal? VlrImpoconsumo1 { get; set; }
        public decimal? VlrImpoconsumo2 { get; set; }
        public decimal? VlrNeto { get; set; }
        public decimal? VlrGravado { get; set; }
        public decimal? VlrDscGravado { get; set; }
        public decimal? VlrExento { get; set; }
        public decimal? VlrDscExento { get; set; }
        public decimal? VlrRetRenta { get; set; }
        public decimal? VlrRetIva { get; set; }
        public decimal? VlrRetIca { get; set; }
        public decimal? VlrRetOtros { get; set; }
        public decimal? VlrRetTotal { get; set; }
        public string IdTercTrans { get; set; }
        public string IdSucTrans { get; set; }
        public string IdCodConductor { get; set; }
        public string IdNomConductor { get; set; }
        public string PlacaVehiculo { get; set; }
        public string Guia { get; set; }
        public decimal? Cajas { get; set; }
        public decimal? Kilos { get; set; }
        public decimal? Volumen { get; set; }
        public decimal? VlrSeg { get; set; }
        public decimal? VlrFleEstan { get; set; }
        public decimal? VlrFleContra { get; set; }
        public string Observacion { get; set; }
        public string NomCliContado { get; set; }
        public string NitCliContado { get; set; }
        public string TipIdeContado { get; set; }
        public string IdTercFa { get; set; }
        public string IdSucFa { get; set; }
        public string NombreFa { get; set; }
        public string IdVendCc { get; set; }
        public string NombreVend { get; set; }
        public string IdCargue { get; set; }
        public string IdCoFv { get; set; }
        public string IdTipoFv { get; set; }
        public string IdNroFv { get; set; }
        public string IdClater1 { get; set; }
        public string IdClater2 { get; set; }
        public string IdClater { get; set; }
        public string DetalleDoc { get; set; }
        public string Detalle1 { get; set; }
        public string Detalle2 { get; set; }
        public string Detalle3 { get; set; }
        public string Detalle4 { get; set; }
        public string Detalle5 { get; set; }
        public string Detalle6 { get; set; }
        public string Detalle7 { get; set; }
        public string Detalle8 { get; set; }
        public string Detalle9 { get; set; }
        public string Detalle10 { get; set; }
        public string DctoFact { get; set; }
        public string NomTransp { get; set; }
        public string FechaGen { get; set; }
        public string PtoenvCod { get; set; }
        public string IdTercCargue { get; set; }
        public string IdSucCargue { get; set; }
        public string IdCondCargue { get; set; }
        public string PlacaCargue { get; set; }
        public string Deta1Cargue { get; set; }
        public string Deta2Cargue { get; set; }
        public string NombreTransCargue { get; set; }
        public string FechaCargueCierre { get; set; }
        public string FichaTecnica { get; set; }
        public string HoraIngDocrm { get; set; }
        public string Campo01FtDoc { get; set; }
        public string FechaFv { get; set; }
        public string HoraIngFv { get; set; }
    }
}
