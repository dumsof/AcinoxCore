using System;
using System.Collections.Generic;

namespace File.Repositorie.DataAccess
{
    public partial class EncabezadoCompras
    {
        public string IdTerc { get; set; }
        public string IdSuc { get; set; }
        public string FechaDcto { get; set; }
        public string IdEmp { get; set; }
        public string IdCo { get; set; }
        public string IdTipo { get; set; }
        public string IdNro { get; set; }
        public string DctoProv { get; set; }
        public string LapsoDcto { get; set; }
        public string NatCxp { get; set; }
        public string FechaDocProv { get; set; }
        public string Detalle { get; set; }
        public string Nitpro { get; set; }
        public string Nompro { get; set; }
        public string DocAlt { get; set; }
        public string CondPago { get; set; }
        public string FormaPago { get; set; }
        public decimal? Periodicidad { get; set; }
        public string ModoCredito { get; set; }
        public decimal? NroCuotas { get; set; }
        public string FechaVcto { get; set; }
        public decimal? Diasg { get; set; }
        public decimal? ValorCuotaIni { get; set; }
        public decimal? ValorCuota { get; set; }
        public decimal? PordesPp { get; set; }
        public decimal? VlrdesPp { get; set; }
        public string Moneda { get; set; }
        public decimal? TasaConver { get; set; }
        public decimal? TasaCambio { get; set; }
        public decimal? VlrtotBru { get; set; }
        public decimal? VlrdesLinea1 { get; set; }
        public decimal? VlrdesLinea2 { get; set; }
        public decimal? VlrdesGlobal1 { get; set; }
        public decimal? VlrdesGlobal2 { get; set; }
        public decimal? Vlriva { get; set; }
        public decimal? Vlrimpoconsumo1 { get; set; }
        public decimal? Vlrimpoconsumo2 { get; set; }
        public decimal? Vlrnet { get; set; }
        public decimal? VlrdifAjuste { get; set; }
        public decimal? Vlrret { get; set; }
        public string IndRegimenSimp { get; set; }
        public decimal? Vlrcxp { get; set; }
        public decimal? Vlrant { get; set; }
        public string CuentaCxp { get; set; }
        public string CoCxp { get; set; }
        public string CcostoCxp { get; set; }
        public string CuentaAnt { get; set; }
        public string CoAnt { get; set; }
        public string CcostoAnt { get; set; }
        public string CuentaDif { get; set; }
        public string CoDif { get; set; }
        public string CcostoDif { get; set; }
        public string ProyectoDif { get; set; }
        public string IndAnulado { get; set; }
        public string EstadoTrans { get; set; }
        public string EstadoContab { get; set; }
        public string Lote { get; set; }
        public string TipdocProv { get; set; }
        public string ProyectoCxp { get; set; }
        public string ProyectoAnt { get; set; }
        public decimal? PorcComision { get; set; }
        public decimal? Vlrcom { get; set; }
        public string TercAnt { get; set; }
        public string SucAnt { get; set; }
        public string Cptofe { get; set; }
        public string ClaseP { get; set; }
        public string FecAplicaPp { get; set; }
        public string FecRadicacion { get; set; }
        public decimal? DiasPp { get; set; }
        public string IndAnticipo { get; set; }
        public string TercCxp { get; set; }
        public string SucCxp { get; set; }
        public string IndConsignacion { get; set; }
        public string GranContribP { get; set; }
        public string IndReterentaP { get; set; }
        public string CodReteivaP { get; set; }
        public string CodReteicaP { get; set; }
        public string CodReteotrP { get; set; }
        public decimal? VlrRetFte { get; set; }
        public decimal? VlrRetIva { get; set; }
        public decimal? VlrRetIca { get; set; }
        public decimal? VlrRetOtros { get; set; }
    }
}
